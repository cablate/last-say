...
const INSTRUMENT_LABELS = Object.freeze({ stock: '股票', etf: 'ETF', mutual_fund: '共同基金', bond: '債券', cash_equivalent: '約當現金', simple_crypto: '簡單加密資產', quoted_asset: '其他有報價資產' });
...
function EntryDialog({ kind, open, accounts, instruments, onOpenChange, onSaved }) {
  ...
  const descriptions = {
    ...
    holding: '手動持倉會同時建立 user-confirmed 來源證據與 snapshot。',
    quote: '手動報價會標記為 manual estimate，不會冒充官方市場資料。',
  ...
  return <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{titles[kind]}</DialogTitle><DialogDescription>{descriptions[kind]}</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit} aria-describedby={error ? 'investment-entry-error' : undefined}>
    ...
    {kind === 'holding' ? <>
      <div className="space-y-2"><Label htmlFor="holding-account">投資帳戶</Label><Select value={form.account_key || ''} onValueChange={(value) => field('account_key', value)}><SelectTrigger id="holding-account" className="w-full"><SelectValue placeholder="選擇投資帳戶" /></SelectTrigger><SelectContent>{investmentAccounts.map((account) => <SelectItem key={account.account_key} value={account.account_key}>{account.display_name} · {account.currency}</SelectItem>)}</SelectContent></Select>{investmentAccounts.length === 0 ? <p className="text-xs text-muted-foreground">請先在帳戶分頁新增「投資帳戶」。</p> : null}</div>
      <div className="space-y-2"><Label htmlFor="holding-instrument">投資工具</Label><Select value={form.instrument_key || ''} onValueChange={(value) => field('instrument_key', value)}><SelectTrigger id="holding-instrument" className="w-full"><SelectValue placeholder="選擇已建立工具" /></SelectTrigger><SelectContent>{instruments.map((item) => <SelectItem key={item.instrument_key} value={item.instrument_key}>{item.name} · {item.quote_currency}</SelectItem>)}</SelectContent></Select>{instruments.length === 0 ? <p className="text-xs text-muted-foreground">請先新增投資工具。</p> : null}</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="holding-value">來源報告市值（{instrumentCurrency}，選填）</Label><Input id="holding-value" inputMode={currencyInputMode(instrumentCurrency)} value={form.reported_market_value || ''} onChange={(event) => field('reported_market_value', event.target.value)} placeholder={currencyInputPlaceholder(instrumentCurrency)} /></div><div className="space-y-2"><Label htmlFor="holding-cost">來源報告成本（{instrumentCurrency}，選填）</Label><Input id="holding-cost" inputMode={currencyInputMode(instrumentCurrency)} value={form.reported_cost_basis || ''} onChange={(event) => field('reported_cost_basis', event.target.value)} placeholder={currencyInputPlaceholder(instrumentCurrency)} /></div></div>
    </> : null}

    {kind === 'quote' ? <>
      <div className="space-y-2"><Label htmlFor="quote-instrument">投資工具</Label><Select value={form.instrument_key || ''} onValueChange={(value) => field('instrument_key', value)}><SelectTrigger id="quote-instrument" className="w-full"><SelectValue placeholder="選擇已建立工具" /></SelectTrigger><SelectContent>{instruments.map((item) => <SelectItem key={item.instrument_key} value={item.instrument_key}>{item.name} · {item.quote_currency}</SelectItem>)}</SelectContent></Select></div>
      <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="quote-price">每單位價格（{instrumentCurrency}）</Label><Input id="quote-price" inputMode="decimal" value={form.price_decimal || ''} onChange={(event) => field('price_decimal', event.target.value)} placeholder="例如 101.23" required /></div><div className="space-y-2"><Label htmlFor="quote-date">報價日期</Label><Input id="quote-date" type="date" value={form.as_of_date || ''} onChange={(event) => field('as_of_date', event.target.value)} required /></div></div>
    </> : null}

    {kind === 'fx' ? <>
      <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="fx-base">基準幣</Label><Select value={form.base_currency} onValueChange={(value) => field('base_currency', value)}><SelectTrigger id="fx-base" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{SUPPORTED_CURRENCIES.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="fx-quote">報價幣</Label><Select value={form.quote_currency} onValueChange={(value) => field('quote_currency', value)}><SelectTrigger id="fx-quote" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{SUPPORTED_CURRENCIES.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></div></div>
...
export default function InvestmentRegister({ inventory, onSaved }) {
  ...
  return <div className="space-y-8">
    ...
    <section className="space-y-3" aria-labelledby="investment-actions-title"><div><h2 id="investment-actions-title" className="font-semibold">人工估值資料</h2><p className="mt-1 text-sm text-muted-foreground">適合先建立目前持倉與估值；正式券商報表與交易仍建議由 AI 走 preview／commit 匯入。</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setDialog('instrument')}><Plus aria-hidden="true" />投資工具</Button><Button size="sm" variant="outline" onClick={() => setDialog('holding')}><Plus aria-hidden="true" />持倉快照</Button><Button size="sm" variant="outline" onClick={() => setDialog('quote')}><Plus aria-hidden="true" />市場報價</Button><Button size="sm" variant="outline" onClick={() => setDialog('fx')}><Plus aria-hidden="true" />匯率</Button></div></section>
