...
import { displayAuthority, displayReadinessGap, displaySourceKind } from '@/lib/finance/presentation-labels';
...
function ReadinessRow({ goal, readiness }) {
  ...
  return <article className="grid gap-3 px-4 py-4 md:grid-cols-[minmax(10rem,0.65fr)_minmax(0,1.35fr)_auto] md:items-center">
    <div className="flex items-center gap-2"><Icon className="size-4 text-muted-foreground" aria-hidden="true" /><h3 className="font-medium">{GOALS[goal] || '其他分析目標'}</h3></div>
    <div className="min-w-0"><p className="text-sm">{gap ? displayReadinessGap(gap.gap) : '必要資料與範圍已通過目前規則。'}</p><p className="mt-1 text-xs text-muted-foreground">{gap ? '下一步：補齊資料後重新檢查。' : `截至 ${readiness?.as_of_date || '目前日期'} · ${readiness?.scope?.kind === 'account' ? '單一帳戶' : '全域範圍'}`}</p></div>
...
export default function ReadinessPanel({ inventory }) {
  ...
  return <div className="space-y-7">
    ...
    <section aria-labelledby="source-coverage-title"><div className="mb-3 flex items-center gap-2"><h2 id="source-coverage-title" className="font-semibold">來源覆蓋</h2><span className="text-sm text-muted-foreground">僅顯示聚合資訊，不暴露檔名</span></div>{sourceCoverage.length ? <div className="divide-y rounded-md border">{sourceCoverage.map((item) => <div key={`${item.source_kind}-${item.authority}-${item.status}`} className="grid gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><p className="font-medium">{displaySourceKind(item.source_kind)}</p><p className="text-xs text-muted-foreground">{displayAuthority(item.authority)} · {item.period_start || '未提供起日'} 至 {item.period_end || '未提供迄日'}</p></div><div className="flex items-center gap-2 text-sm tabular-nums"><span>{item.count} 份</span><ArrowRight className="size-4 text-muted-foreground" aria-hidden="true" /></div></div>)}</div> : <p className="rounded-md border border-dashed px-5 py-8 text-center text-sm text-muted-foreground">尚未登錄來源證據。請先由 AI 建立來源與匯入預覽。</p>}</section>
  </div>;
}
