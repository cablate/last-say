...
3. **未覆蓋的明細** → 你分析分類（category）並給**信心度 0~1**。**即使沒把握也要給最佳猜測 + 低信心（0.2~0.4），不要留空標「待確認」**——低信心會排到 UI 審查最前面讓人複核，且 correction_log 才能記「猜測→正確」供你第二環進化規則。截斷／陌生商家名用 websearch 補全，結果寫進 `note`（詳見 `prompts/playbook.md` A3）。
...
- `category_primary`：任意（建議用標準 13 類，見下）
- `memo`：任意文字
...
→ 整理成 corrections: [{id:1, category_primary:"飲食"}, {id:2, category_primary:"飲食"}, ...]
