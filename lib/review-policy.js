...
function ownerUnresolvedSql(alias = '') {
  return `${column(alias, 'category_primary')} = '${OWNER_UNRESOLVED_CATEGORY}'`;
}

function isOwnerUnresolvedRow(row) {
  return String(row?.category_primary || '').trim() === OWNER_UNRESOLVED_CATEGORY;
}

module.exports = {
  LOW_CONFIDENCE_REVIEW_THRESHOLD,
  ownerUnresolvedSql,
  isOwnerUnresolvedRow,
  needsReviewSql,
  needsReviewRow,
};
