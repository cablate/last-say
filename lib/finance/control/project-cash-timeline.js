...
function projectCashTimeline(input) {
  ...
  const policy = input.policy || { status: 'evaluated' };
  plainObject(policy, 'policy');
  if (!['evaluated', 'unavailable'].includes(policy.status)) throw new TypeError('policy.status is invalid');
  const opening = integerMinor(input.opening_liquid_cash_minor, 'opening_liquid_cash_minor');
  const reserve = policy.status === 'unavailable' ? 0n : integerMinor(input.reserve_floor_minor, 'reserve_floor_minor');
  const buffer = policy.status === 'unavailable' ? 0n : integerMinor(input.uncertainty_buffer_minor ?? '0', 'uncertainty_buffer_minor');
  ...
  for (let offset = 0; offset < horizonDays; offset += 1) {
    ...
    current += net;
    const headroom = policy.status === 'unavailable' ? null : current - reserve - buffer;
    if (current < minimum) { minimum = current; minimumDate = date; }
    if (policy.status !== 'unavailable' && !firstBreach && current < reserve + buffer) firstBreach = date;
    daily.push({
      date,
      ...
      reserve_floor_minor: policy.status === 'unavailable' ? null : reserve.toString(),
      uncertainty_buffer_minor: policy.status === 'unavailable' ? null : buffer.toString(),
      headroom_minor: headroom === null ? null : headroom.toString(),
    });
  }
  ...
  const safeToSpend = policy.status === 'evaluated' && coverageStatus === 'complete'
  ...
  return {
    ...
    policy: { status: policy.status, reason: policy.reason || null },
    daily,
    summary: {
