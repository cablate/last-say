...
// SQLite schema compatibility marker. Increment only with a documented migration.
// A newer DB must never be opened by an older app version that may not understand it.
...
const SCHEMA_SQL = `
  ...
  CREATE INDEX IF NOT EXISTS idx_rule_change_log_rule ON rule_change_log(rule_id);
  CREATE TRIGGER IF NOT EXISTS rule_change_log_no_update
  BEFORE UPDATE ON rule_change_log
  BEGIN
    SELECT RAISE(ABORT, 'rule_change_log is append-only');
  END;
...
// initializeDatabase：建表與相容性 migration 必須同進同退，避免留下半升級 schema。
function initializeDatabase(db) {
  const currentVersion = getSchemaVersion(db);
  if (currentVersion > SCHEMA_VERSION) {
    throw new Error(
      `資料庫 schema 版本 ${currentVersion} 高於本程式支援的 ${SCHEMA_VERSION}；請升級 Last Say，勿用舊版開啟此資料庫。`,
    );
  }
  ...
  withImmediateTransaction(db, () => {
    ...
    runMigrations(db, MIGRATIONS, { appVersion: APP_VERSION });
    if (currentVersion !== SCHEMA_VERSION) {
...
function applySchemaMigrations(db) {
  ...
  if (tableExists(db, 'transactions')) {
    // Read-only analysis paths may open an already-migrated database. Avoid
    // issuing a no-op UPDATE on every read request; SQLite still treats it as
    // a write and can reject it when the runtime cannot write the database.
    const pendingHumanReviews = db.prepare(
      "SELECT COUNT(*) AS count FROM transactions WHERE reviewed = 0 AND classification_source = 'human'",
    ).get().count;
    if (Number(pendingHumanReviews) > 0) {
      db.exec("UPDATE transactions SET reviewed = 1 WHERE reviewed = 0 AND classification_source = 'human'");
    }
  }
}

// 公開 migration 入口同樣保證原子性；若呼叫端已在 transaction 內，沿用該 transaction。
function migrateSchema(db) {
  return withImmediateTransaction(db, () => {
    applySchemaMigrations(db);
    // Partial/very old exports may not have the baseline parent tables. Keep
    // the legacy compatibility behavior; initializeDatabase will create the
    // baseline and run the ledger when this becomes a complete application DB.
    if (tableExists(db, 'accounts') && tableExists(db, 'sources')) {
      runMigrations(db, MIGRATIONS, { appVersion: APP_VERSION });
      db.exec(`PRAGMA user_version = ${SCHEMA_VERSION}`);
    }
  });
...
function createDb() {
  ...
  try {
    ...
    try { db.close(); } catch { /* initialization already closed it */ }
    throw error;
  }
}
