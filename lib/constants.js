...
// 使用者已確認「現金移動存在，但用途無法復原」的暫記分類。
// 這不是 AI 低信心 fallback；它只應由人類明確選擇，並在報表中保持揭露。
const OWNER_UNRESOLVED_CATEGORY = '無法確認';

// 標準主類別白名單（題二發散度控制）：AI 分類的 category_main 須對應其一（只能選、不能造）。
// 無法對應 → 用決策樹歸日常開銷/購物並標低信心；AI 不得自行選「無法確認」。
const STANDARD_CATEGORIES = [
  '飲食', '日常開銷', '居住', '交通', '購物', '休閒娛樂', '訂閱服務',
  '醫療保健', '保險', '教育學習', '金融手續與稅費',
  '轉帳/內部移轉', '薪資收入', '其他收入與收益', OWNER_UNRESOLVED_CATEGORY,
];
