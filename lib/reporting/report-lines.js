...
function builtInReportLine(row) {
  ...
  if (Number(row.typed_transfer_amount_minor) > 0) {
    return reportExclusionForEventKind('own_transfer');
  }

  if (Number(row.typed_card_payment_amount_minor) > 0) {
    return reportExclusionForEventKind('credit_card_payment');
  }

  if (Number(row.typed_investment_cash_minor) > 0) {
    return reportExclusionForEventKind('investment_purchase');
  }
  ...
  if (hasAny(text, KEYWORDS.loanPrincipal)) {
    return reportExclusionForEventKind('loan_principal');
  }
