...
function validateRule(data) {
  ...
  if (cat === OWNER_UNRESOLVED_CATEGORY) {
    throw new Error('無法確認只能由擁有者逐筆決定，不能建立可重複套用的分類規則');
  }
  return {
    $mk: mk, $st: st, $dir: dir,
    $cat: cat,
...
function createRule(data) {
  ...
  // category_value 軟校驗：非標準 15 類仍接受（不破壞彈性），但附 warning 提示對齊。
  // 不硬擋——AI 可能有特殊情境需要自訂類別，但偏離標準會讓報表映射/統計彙總失效。
  if (!STANDARD_CATEGORIES.includes(rule.$cat)) {
    created.warning = `category_value「${rule.$cat}」非標準類別，建議對齊 15 類（見 GET /api/meta 回應 standardCategories）以利報表映射與統計彙總。`;
  }
  return created;
}
