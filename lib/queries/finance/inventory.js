  ...
  if (goal === 'debt_obligations') {
    ...
    for (const liability of liabilities) {
      if (!liability.start_date) {
        gaps.push({ gap: 'missing_loan_start_date', account_key: liability.account_key, liability_key: liability.liability_key, next_action: 'add_verified_loan_start_date' });
      }
      const principal = db.prepare(`SELECT snapshot_key,as_of_date,authority,review_state FROM account_balance_snapshots
      ...
      else if (principal.review_state !== 'confirmed' || ['estimated', 'ai_inferred'].includes(principal.authority)) {
        gaps.push({ gap: 'loan_principal_needs_review', account_key: liability.account_key, liability_key: liability.liability_key, snapshot_key: principal.snapshot_key, next_action: 'confirm_or_replace_principal_balance' });
      }
    }
