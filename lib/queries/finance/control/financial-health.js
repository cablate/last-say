...
function remainingCardMinimumDue(card) {
  if (card.unpaid_due_minor === null || card.unpaid_due_minor === undefined) return null;
  const unpaid = BigInt(card.unpaid_due_minor);
  if (unpaid <= 0n) return '0';
  const minimum = card.statement?.minimum_due_minor;
  if (minimum === null || minimum === undefined) return null;
  const minimumValue = BigInt(minimum);
  return (minimumValue < unpaid ? minimumValue : unpaid).toString();
}
...
function compactLiability(profile, currentByAccount, asOfDate, reportCurrency) {
  ...
  const remainingScheduleEntryCount = (profile.schedule || [])
    .filter((entry) => entry.due_date && entry.due_date >= asOfDate && entry.entry_status !== 'settled')
    .length;
  ...
  return {
    ...
    maturity_date: profile.maturity_date || null,
    ...
    remaining_schedule_entry_count: remainingScheduleEntryCount,
  ...
  const facts = {
    ...
    liquidity: {
      ...
      known_monthly_debt_service_minor: sumMinor([
        ...
        ...cards.map(remainingCardMinimumDue),
      ]),
