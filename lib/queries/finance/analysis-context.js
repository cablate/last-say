...
const { getSpendingStructure } = require('./control/spending-structure');
const { getFinancialDashboardHistory } = require('./control/history');
const { getObligationTimeline } = require('./control/obligations');
const { getCashForecast } = require('./control/forecast');
...
function spendingStructure(spec, entity, baseCurrency, db) {
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(spec.month || '')) {
    throw new FinanceError('VALIDATION_ERROR', 'spending_structure requires month=YYYY-MM', { field: 'month' });
  }
  const model = getSpendingStructure(new URLSearchParams({
    month: spec.month,
    entity_id: entity,
    currency: spec.currency || baseCurrency,
    basis: spec.basis || 'card_accrual_management',
  }), db);
  return { data: model, provenance: provenance(db, model.source_watermark?.resource_keys || []) };
}

function financialDashboardHistory(spec, entity, baseCurrency, db) {
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(spec.month || '')) {
    throw new FinanceError('VALIDATION_ERROR', 'financial_dashboard_history requires month=YYYY-MM', { field: 'month' });
  }
  const model = getFinancialDashboardHistory(new URLSearchParams({
    month: spec.month,
    entity_id: entity,
    currency: spec.currency || baseCurrency,
    basis: spec.basis || 'card_accrual_management',
  }), db);
  return { data: model, provenance: provenance(db, model.source_watermark?.fact_keys || []) };
}

function obligationTimeline(spec, entity, baseCurrency, db) {
  const query = new URLSearchParams({
    entity_id: entity,
    currency: spec.currency || baseCurrency,
    as_of_date: spec.as_of_date || new Date().toLocaleDateString('en-CA'),
    horizon_days: String(spec.horizon_days || 90),
  });
  const model = getObligationTimeline(query, db);
  return { data: model, provenance: provenance(db, model.source_watermark?.resource_keys || []) };
}

function cashForecast(spec, entity, baseCurrency, db) {
  const query = new URLSearchParams({
    entity_id: entity,
    currency: spec.currency || baseCurrency,
    as_of_date: spec.as_of_date || new Date().toLocaleDateString('en-CA'),
  ...
  const model = getCashForecast(query, db);
  return { data: model, provenance: provenance(db, model.source_watermark?.resource_keys || []) };
}
...
function analysisContext(input, db = getDb()) {
  ...
  const datasets = input.datasets.map(validateSpec).map((spec) => {
    ...
    if (spec.name === 'spending_structure') result = spendingStructure(spec, entityKey, entity.base_currency, db);
    if (spec.name === 'financial_dashboard_history') result = financialDashboardHistory(spec, entityKey, entity.base_currency, db);
    if (spec.name === 'obligation_timeline') result = obligationTimeline(spec, entityKey, entity.base_currency, db);
    if (spec.name === 'cash_forecast') result = cashForecast(spec, entityKey, entity.base_currency, db);
