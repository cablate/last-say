...
function conflictItem(task, db) {
  ...
  }, '選擇較可信的來源證據', { financial: [], timelines: [] });
  ...
  item.before = { target_context: conflict.target_context, semantic_key: conflict.semantic_key, status: conflict.status };
...
function ownerUnresolvedItems(db) {
  return db.prepare(`SELECT t.id,t.transaction_key,t.transaction_date,t.name,t.currency,t.flow_type,
      ...
      a.account_key,COALESCE(a.display_name,a.name) AS account_name
    FROM transactions t JOIN accounts a ON a.id=t.account_id
    WHERE ${activeRecordSql('t')} AND ${ownerUnresolvedSql('t')}
    ORDER BY t.transaction_date DESC,t.id DESC LIMIT 200`).all().map((row) => ({
...
function reviewWorkbench(db = getDb(), now = new Date()) {
  const sections = {
    human_confirmations: listHumanConfirmations({ status: 'pending' }, db).map(humanConfirmationItem),
    actionable_reviews: [],
    owner_unresolved: ownerUnresolvedItems(db),
    conflicts: [],
  };
  const partialErrors = [];
