...
function createSourceConflict(input, actor = {}, db = getDb()) {
  ...
  return withTransaction(db, () => {
    db.prepare(`INSERT INTO source_conflicts(conflict_key,target_context,semantic_key,left_source_id,right_source_id,authority,review_state)
      VALUES(?,?,?,?,?,?,?)`).run(key, enumValue(input.target_context, 'target_context', 'target_context'), requiredString(input.semantic_key, 'semantic_key', 300), first.id, second.id, enumValue(input.authority, 'authority', 'authority'), enumValue(input.review_state, 'review_state', 'review_state', 'needs_review'));
    const row = getSourceConflict(key, db);
    createReviewTask({ task_kind: 'source_conflict', resource_type: 'source_conflict', resource_key: key, source_conflict_id: row.id, priority: 90, reason: `Choose authoritative evidence for ${row.target_context}:${row.semantic_key}` }, db);
