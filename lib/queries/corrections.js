...
function getCorrectionFeedbackLoop() {
  const db = getDb();
  const corrections = db.prepare('SELECT COUNT(*) AS count FROM correction_log').get().count || 0;
  ...
  return {
    corrections,
    humanCorrectionRules: rules.count || 0,
    autoApplied: rules.applied || 0,
  };
}
...
function getCorrections({ limit = 200, field = '', matchKey = '' } = {}) {
  ...
  const rows = db.prepare(
    `SELECT cl.*, t.name AS transaction_name, t.transaction_date, t.outflow AS transaction_outflow, t.amount AS transaction_amount FROM correction_log cl JOIN transactions t ON t.id = cl.transaction_id${where} ORDER BY cl.corrected_at DESC, cl.id DESC LIMIT $limit`
  ).all({ ...params, $limit: safeLimit });
  const summary = getCorrectionSummary();
  return { rows, summary, total, feedbackLoop: getCorrectionFeedbackLoop() };
}

module.exports = { logCorrection, getCorrectionSummary, getCorrectionFeedbackLoop, getCorrections };
