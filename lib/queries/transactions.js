...
function getMeta() {
  ...
  return {
    ...
    counts: {
      ...
      needsReview: db.prepare(`SELECT COUNT(*) AS count FROM transactions WHERE ${needsReviewSql()}`).get().count,
    },
    months: { transaction: transactionMonths },
    // 標準 14 類白名單（AI 分類須對齊 Finance Viewer Skill 的 category guide）。
