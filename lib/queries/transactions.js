...
const SPEND_WHERE = `
  outflow > 0
  AND ${activeRecordSql()}
  AND (flow_type IS NULL OR flow_type NOT IN (${NON_SPEND_FLOW_IN_LIST}))
`;
...
function getMeta() {
  ...
  const transactionMonths = db.prepare(`
    ...
    FROM transactions WHERE ${activeRecordSql()} GROUP BY transaction_month ORDER BY transaction_month
  `).all();
