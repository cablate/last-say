...
const { needsReviewSql } = require('../review-policy');
...
function buildTransactionWhere(params, tableAlias = '') {
  ...
  if (params.get('view') !== 'history') where.push(activeRecordSql(tableAlias));
  if (month) {
...
function getMeta() {
  ...
  return {
    ...
    // 標準 14 類白名單（AI 分類須對齊 Last Say Skill 的 category guide）。
