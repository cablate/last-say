...
const { needsReviewSql, ownerUnresolvedSql } = require('../review-policy');
...
const {
  ...
  CONFIDENCE_TIERS,
  STANDARD_CATEGORIES,
  OWNER_UNRESOLVED_CATEGORY,
  NON_SPEND_FLOW_TYPES,
} = require('../constants');

// 把 NON_SPEND_FLOW_TYPES 注入為 SQL IN list（常數陣列非使用者輸入）。
const NON_SPEND_FLOW_IN_LIST = NON_SPEND_FLOW_TYPES.map((v) => "'" + String(v).replace(/'/g, "''") + "'").join(', ');
// 「實際消費」認定條件：outflow>0 且 flow_type 不在非消費清單（NULL 視為消費，與 reports excluded 對齊）。
const SPEND_WHERE = `
  outflow > 0
  AND ${activeRecordSql()}
  AND (
    flow_type IS NULL
    OR flow_type NOT IN (${NON_SPEND_FLOW_IN_LIST})
    OR EXISTS (
      SELECT 1
      FROM transaction_report_mappings trm
      WHERE trm.transaction_id = transactions.id
        AND trm.report_line LIKE 'expense:%'
    )
  )
`;
...
function buildTransactionWhere(params, tableAlias = '', options = {}) {
  const where = [];
  const values = {};
  ...
  if (view !== 'history') where.push(activeRecordSql(tableAlias));
  if (month) {
    where.push(`${prefix}transaction_month = $month`);
    values.$month = month;
  }

  const ledgerVisibilityView = ['history', 'review', 'unreviewed', 'needs-review', 'unresolved'].includes(view);
  const includeExcludedCashMovements = Boolean(
    options.includeExcludedCashMovements
    || ledgerVisibilityView
    || params.get('ids')
    || category === OWNER_UNRESOLVED_CATEGORY
  );
  if (!includeExcludedCashMovements) {
    where.push(`(
      ${prefix}flow_type IS NULL
      OR (${prefix}flow_type <> '信用卡繳款/移轉' AND ${prefix}flow_type <> '信用卡繳款')
      OR EXISTS (
        SELECT 1
        FROM transaction_report_mappings trm
        WHERE trm.transaction_id = ${prefix}id
          AND trm.report_line LIKE 'expense:%'
      )
    )`);
  }
  ...
  // needs-review：與 getMeta/getSummary/reports 共用 review-policy。
  if (view === 'needs-review') where.push(needsReviewSql(tableAlias));
  if (view === 'unresolved') where.push(ownerUnresolvedSql(tableAlias));
...
function getMeta() {
  ...
  const transactionMonths = db.prepare(`
    SELECT
      ...
      SUM(CASE WHEN (
        flow_type IS NULL
        OR flow_type NOT IN ('信用卡繳款/移轉', '信用卡繳款')
        OR EXISTS (
          SELECT 1
          FROM transaction_report_mappings trm
          WHERE trm.transaction_id = transactions.id
      ...
      ) THEN 1 ELSE 0 END) AS visible_rows
    FROM transactions
    WHERE ${activeRecordSql()}
    GROUP BY transaction_month
  ...
  return {
    ...
    counts: {
      ...
      // 待人類處理：未審 + AI/pending 沒把握；規則列由規則績效管理，不混進 AI 待審。
      needsReview: db.prepare(`SELECT COUNT(*) AS count FROM transactions WHERE ${activeRecordSql()} AND ${needsReviewSql()}`).get().count,
      ownerUnresolved: db.prepare(`SELECT COUNT(*) AS count FROM transactions WHERE ${activeRecordSql()} AND ${ownerUnresolvedSql()}`).get().count,
    },
    months: { transaction: transactionMonths },
    // 標準 15 類白名單（AI 分類須對齊 Last Say Skill 的 category guide）。
    // 與 filters.categories（DB 實際出現過的 distinct 值）區分：前者是規範、後者是現況。
...
function getSummary(params, db = getDb()) {
  const { sql, values } = buildTransactionWhere(params);
  const classificationFilter = buildTransactionWhere(params, '', { includeExcludedCashMovements: true });
  const month = selectedMonth(params);
  ...
  const classification = db.prepare(`
    SELECT
      ...
      COALESCE(SUM(CASE WHEN ${needsReviewSql()} THEN 1 ELSE 0 END), 0) AS needsReview,
      COALESCE(SUM(CASE WHEN ${ownerUnresolvedSql()} THEN 1 ELSE 0 END), 0) AS ownerUnresolved,
      COALESCE(SUM(CASE WHEN ${ownerUnresolvedSql()} THEN inflow ELSE 0 END), 0) AS ownerUnresolvedInflow,
      COALESCE(SUM(CASE WHEN ${ownerUnresolvedSql()} THEN outflow ELSE 0 END), 0) AS ownerUnresolvedOutflow,
      COALESCE(SUM(CASE WHEN ${ownerUnresolvedSql()} THEN inflow - outflow ELSE 0 END), 0) AS ownerUnresolvedNet
    FROM transactions ${classificationFilter.sql}
  `).get(classificationFilter.values);
  const latestBalance = db.prepare(`
    SELECT balance, transaction_date AS date, name
    FROM transactions
    WHERE balance IS NOT NULL AND source_type LIKE '%帳戶%' AND ${activeRecordSql()}
...
function getBreakdown(params) {
  ...
  // The ordinary breakdown intentionally hides settlement/transfer-shaped cash.
  // For the owner-only unresolved bucket, the gross cash movement itself is the
  // important disclosure, so replace its partial row with a complete ledger row.
  const requestedCategory = params.get('category');
  if (
    dimension === 'category'
    && (!requestedCategory || requestedCategory === OWNER_UNRESOLVED_CATEGORY)
  ) {
    const unresolvedParams = new URLSearchParams(params);
    unresolvedParams.set('category', OWNER_UNRESOLVED_CATEGORY);
    const unresolvedFilter = buildTransactionWhere(
      unresolvedParams,
      '',
      { includeExcludedCashMovements: true },
    );
    const unresolved = db.prepare(`
      SELECT
        category_primary AS label,
        COUNT(*) AS rows,
        COALESCE(SUM(inflow), 0) AS inflow,
        COALESCE(SUM(outflow), 0) AS outflow,
        COALESCE(SUM(CASE WHEN ${SPEND_WHERE} THEN outflow ELSE 0 END), 0) AS spend,
        COALESCE(SUM(amount), 0) AS signedTotal
      FROM transactions ${unresolvedFilter.sql}
    `).get(unresolvedFilter.values);
    if (unresolved?.rows > 0) {
      const existingIndex = rows.findIndex((row) => row.label === OWNER_UNRESOLVED_CATEGORY);
      if (existingIndex >= 0) rows[existingIndex] = unresolved;
      else rows.push(unresolved);
      rows.sort((a, b) => (
        (b.spend - a.spend)
        || (b.outflow - a.outflow)
        || (b.rows - a.rows)
...
function getSpending(month, category) {
  const db = getDb();
  const where = ['outflow > 0', activeRecordSql()];
  const params = {};
...
function markReviewed(items, db = getDb()) {
  if (!Array.isArray(items) || items.length === 0) return { reviewed: 0, conflicts: [] };
  const MAX = 500;
  const work = items.slice(0, MAX).map((item) => (
    item && typeof item === 'object'
      ? { id: Number(item.id), expected_updated_at: item.expected_updated_at || null }
      : { id: Number(item), expected_updated_at: null }
  )).filter((item) => Number.isFinite(item.id));
  if (work.length === 0) return { reviewed: 0, conflicts: [] };
  const conflicts = [];
  for (const item of work) {
    const current = db.prepare('SELECT id,updated_at,reviewed FROM transactions WHERE id=?').get(item.id);
    if (!current) conflicts.push({ id: item.id, code: 'NOT_FOUND' });
    else if (item.expected_updated_at && current.updated_at !== item.expected_updated_at) {
      conflicts.push({ id: item.id, code: 'VERSION_CONFLICT', expected_updated_at: item.expected_updated_at, actual_updated_at: current.updated_at });
    }
  }
  if (conflicts.length > 0) return { reviewed: 0, conflicts, truncated: items.length > MAX };
  db.exec('BEGIN');
  try {
    let reviewed = 0;
    for (const item of work) {
      const result = item.expected_updated_at
        ? db.prepare('UPDATE transactions SET reviewed=1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND updated_at=?').run(item.id, item.expected_updated_at)
        : db.prepare('UPDATE transactions SET reviewed=1,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(item.id);
      if (Number(result.changes) !== 1) throw new Error(`Transaction ${item.id} changed during review`);
      reviewed += Number(result.changes);
    }
    db.exec('COMMIT');
    return { reviewed, conflicts: [], truncated: items.length > MAX };
  } catch (error) {
    db.exec('ROLLBACK');
    throw error;
  }
...
function patchTransaction(txnId, body, db = getDb()) {
  ...
  try {
    for (const item of work) {
      try {
        ...
        if (item.expected_updated_at && item.expected_updated_at !== current.updated_at) {
          results.errors++;
          results.details.push({ id: item.id, error: 'version conflict', code: 'VERSION_CONFLICT' });
          continue;
        }
        const logCtx = {
          match_key: normalizeForRule(current.name),
