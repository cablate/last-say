...
function loadRows(db, month, entityId, currency) {
  ...
  return db.prepare(`
    SELECT
      ...
      trm.reviewed AS mapping_reviewed,
    ...
    LEFT JOIN (
      SELECT transaction_id,SUM(amount_minor) amount_minor,GROUP_CONCAT(match_key) match_keys FROM (
        SELECT from_transaction_id transaction_id,amount_minor,match_key
        FROM transfer_matches
        WHERE match_status='confirmed' AND review_state='confirmed'
        UNION ALL
        SELECT to_transaction_id transaction_id,amount_minor,match_key
        FROM transfer_matches
        WHERE match_status='confirmed' AND review_state='confirmed' AND to_transaction_id IS NOT NULL
      ) GROUP BY transaction_id
    ) tm ON tm.transaction_id=t.id
    LEFT JOIN (
      SELECT transaction_id,SUM(amount_minor) amount_minor,GROUP_CONCAT(match_key) match_keys
      FROM credit_card_payment_matches
      WHERE record_status NOT IN ('reversed','superseded','archived')
        AND review_state='confirmed'
        AND match_status IN ('confirmed','partial','settled')
      GROUP BY transaction_id
...
function getIncomeStatement(params, db = getDb()) {
  ...
  const unmappedTransactionKeys = new Set();
  ...
  for (const row of rows) {
    ...
    if (typedLoanTotal > 0 && !row.mapping_report_line) {
      ...
      if (remainder > 0) {
        unmappedCount += 1;
        if (row.transaction_key) unmappedTransactionKeys.add(row.transaction_key);
  ...
  const unmatchedTransferCount = listUnmatchedTransferCandidates(db)
    .filter((candidate) => scopedTransactionKeys.has(candidate.transaction_key)
      && !unmappedTransactionKeys.has(candidate.transaction_key)).length;
