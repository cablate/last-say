...
function upsertTransactionReportMapping(data, db = getDb()) {
  ...
  // reason/note 未明確提供時（=== undefined）保留舊值；明確給 null 代表清空。
  ...
  // note 附加到 reason 的合成僅在兩者其一被明確提供時才做
  const reasonProvidedOrNote = reasonProvided || noteProvided;
  ...
  // 但未「明確提供」的 confidence/reason/note/mapping_source 保留舊值，避免被抹成 null。
  const existing = db.prepare(`
    SELECT mapping_source, confidence, reason FROM transaction_report_mappings WHERE transaction_id = ?
  `).get(transactionId);

  let finalMappingSource = mappingSource;
  let finalConfidence = confidence;
  let finalReason = null;
  if (reasonProvidedOrNote) {
    finalReason = note ? (reason ? `${reason}\n${note}` : note) : reason;
  }
  ...
  if (existing) {
    ...
    // reason：本次 reason/note 皆未提供 → 保留舊值
    if (!reasonProvidedOrNote && existing.reason != null) {
      finalReason = existing.reason;
    }
    db.prepare(`
      UPDATE transaction_report_mappings
      SET report_line = $line, mapping_source = $src, confidence = $conf, reason = $reason
