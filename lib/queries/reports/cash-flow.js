...
function classifyTypedOwner(kind, row, maps, reportCurrency, sections, state) {
  ...
  if (kind === 'card_settlement') {
    ...
    // Typed card-payment writes use settled/partial. Keep legacy confirmed rows
    // readable, but never treat a partial allocation as a complete settlement.
    const confirmed = matches.filter((match) => ['settled', 'confirmed'].includes(match.match_status));
