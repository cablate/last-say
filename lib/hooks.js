...
export function useFinancialHealth(params, enabled = true) {
  return useApi(
    (signal) => enabled
      ? fetchJson(withQuery("/api/finance/control/financial-health", params), { signal })
      : Promise.resolve(null),
    [params, enabled],
  )
}

export function useSpendingStructure(params, enabled = true) {
  return useApi(
    (signal) => enabled
      ? fetchJson(withQuery("/api/finance/control/spending-structure", params), { signal })
      : Promise.resolve(null),
    [params, enabled],
  )
}

export function useFinancialDashboardHistory(params, enabled = true) {
  return useApi(
    (signal) => enabled
      ? fetchJson(withQuery("/api/finance/control/history", params), { signal })
      : Promise.resolve(null),
    [params, enabled],
  )
}

export function useObligationTimeline(params, enabled = true) {
  return useApi(
    (signal) => enabled
      ? fetchJson(withQuery("/api/finance/control/obligations", params), { signal })
      : Promise.resolve(null),
    [params, enabled],
  )
}

export function useCashForecast(params, enabled = true) {
  return useApi(
    (signal) => enabled
      ? fetchJson(withQuery("/api/finance/control/forecast", params), { signal })
      : Promise.resolve(null),
    [params, enabled],
  )
}
