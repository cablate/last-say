...
> - 狀態：Canonical spec-plan；Control Center已確認為資料基礎建設之後的下一階段；Phase 0 reference、FC-A1 analysis contract、FC-A2 Monthly Financial Pulse、FA-0 Financial Health Review v0、FC-A3 Spending Structure、FC-2 Obligation Timeline、FC-3 Raw Cash Forecast與foundation R16 card lifecycle已完成第一輪code／synthetic closure，下一步進入真實問題的Context Pack驗收與AI提示詞校準
> - 最近重驗：2026-07-21（原始規劃 2026-07-13；本計畫內容日期仍保留各work package實際完成日）
...
| 當前信用卡暴險 | typed facts、current→posted lifecycle與FC-2 obligation timeline v0已有 | 中高（資料）／中（Control） | profiles、statements、items、payment matches、installments與FC-2 read model已存在；正式資料的即時／未出帳／已出帳／due coverage與owner acceptance仍待驗收。 |
| 貸款契約與還款表 | typed facts與FC-2 projection v0已有 | 中 | liability profile、schedule entries、payment allocations與7／30／90日timeline已存在；缺官方schedule時維持blocker，不猜測。 |
| 固定／週期承諾 | typed templates／occurrences與FC-2 projection v0已有 | 中 | 可保存recurring／one-off facts並投影已確認occurrences；unknown amount／due仍以range／unknown／blocker表達。 |
...
| 每日現金預測 | FC-3 Raw Cash Forecast v0已具bounded runtime | 中（raw path）／無（safe-to-spend） | `control/forecast.js`與`/api/finance/control/forecast`使用trusted opening cash＋FC-2已知義務；不持久化forecast run，未設定reserve／reliable income時不提供safe-to-spend。 |
...
**Verdict：FC-3 execution-ready；FC-A2、FA-0、FC-A3、FC-2與FC-3已提供bounded runtime consumers，下一個切片是以真實問題驗收compact Context Pack與AI提示詞。** `FC-4`之後刻意保留owner policy decision，現在不能假裝safe-to-spend已可用。

已確認的技術基線：repository code與正式DB均為schema v10；15個named analysis datasets、FA-0 Context Pack、proposal envelope、unified review workbench、三張management reports、readiness、preview／commit、browser confirmation、backup／restore、synthetic Control fixture與FC-3 raw cash path均存在。Migration postflight原為1,078筆交易、24個來源與13個帳戶；後續實名資料均在備份／副本演練後寫入，2026-07-21正式DB為1,108筆交易、33個來源、13個帳戶、12個balance snapshots、5個instruments與5個holding snapshots。Card profile／statement／payment、3筆liability、2筆commitment、investment／FX與valued items已進入typed foundation；目前沒有待人確認的proposal。當前Balance Sheet已complete；仍缺owner scope attestations、部分歷史cash boundaries、loan allocations及歷史card normalization closure，因此Control必須保留partial／known-gap狀態。2026-07-21完整release verifier已通過235／235 Node tests、7／7 Chromium、18／18 Skill eval，以及build／runtime／privacy／backup restore等全部gate。
...
| GATE-F3 當前位置 | 每個納入帳戶有dated balance／principal／holding／valuation；FX缺口不被當0 | **Passed through 2026-07-21 position**：正式Balance Sheet為complete、equation delta 0、blockers 0；台股owner-confirmed position已連到canonical instrument與dated official quote。這不代表歷史cash boundary已補齊；精確私人金額只存在ignored evidence zone |
...
| ID | Goal | 主要owner／預計檔案 | Done when |
|---|---|---|---|
| `FC-2A` | 把G2 facts正規化為future events | **Implemented 2026-07-17**：`lib/finance/control/project-obligations.js`與`test/control-obligation-projection.test.js` | card statement／installment、loan schedule、commitment各自映射；stable key、date、amount／unknown、reliability、source facts完整；payment不再算expense；貸款components必須精確對回amount |
| `FC-2B` | 提供7／30／90日read model | **Implemented 2026-07-17**：`lib/queries/finance/control/obligations.js`、`app/api/finance/control/obligations/route.js`、analysis registry／context與`test/control-obligations-api.test.js` | 每筆event可drill back；missing schedule顯示blocker；相同event不重複；query-time deterministic、無新增canonical persistence |
| `FC-2C` | 顯示可審查的義務日曆 | **Implemented 2026-07-17**：`components/financial-control/UpcomingCommitments.jsx`與Control page section | exact／range／unknown／blocker可分辨；修改資料後重新讀API，不在client重建語意；`CashTimeline.jsx`留給FC-3C；browser acceptance仍待補 |
| `FC-2D` | 讓AI能操作新能力 | **Implemented 2026-07-17**：`.claude/skills/last-say-ops/` API／recipe／prompt references | AI先查readiness再讀timeline；只對缺漏facts提出typed action；Skill eval與operator contract既有gate通過，正式資料接受仍待owner驗收 |
...
| ID | Goal | 主要owner／預計檔案 | Done when |
|---|---|---|---|
| `FC-3A` | 分離raw cash projection與財務policy | **Implemented 2026-07-17**：`docs/contracts/cash-forecast-contract.md`、`lib/finance/control/project-cash-timeline.js`與`test/control-cash-timeline.test.js` | 未設定reserve／reliable income時仍可顯示「已知承諾下的raw projection」，但`safe_to_spend`與安全狀態必為unavailable；不以reserve=0偽裝已設定 |
| `FC-3B` | 建立runtime adapter／API | **Implemented 2026-07-17**：`lib/queries/finance/control/forecast.js`、`app/api/finance/control/forecast/route.js`、analysis registry／context與`test/control-forecast-api.test.js` | opening cash只取trusted position；event只取FC-2；同watermark可重現；缺口可預期降級 |
| `FC-3C` | 顯示90日時間軸 | **Implemented 2026-07-17**：`components/financial-control/CashTimeline.jsx`與Control page section | 顯示最低現金點、daily closing path、excluded／blockers與coverage；不先顯示綠色安全判斷；browser acceptance仍待補 |
...
| `OD-FORECAST` | FC-3 runtime後 | as-of／horizon的簡單default；未確認income排除即可 |
...
| 7／30／90日義務表 | 已知卡費、貸款、commitment的日期、金額／range、狀態與來源 | unknown或candidate確認 | **Implemented v0／FC-2**：不需要AI逐次計算；正式資料接受與card lifecycle仍須owner驗收 |
| 90日現金時間軸 | opening cash＋已確認future events的每日餘額、最低點與coverage | AI只解釋；owner稍後決定policy | **Implemented v0／FC-3**：runtime adapter／API／UI已存在，正式資料與owner policy仍待驗收 |
...
| `FC-A3` | **Implemented 2026-07-17**：支出結構read model／UI：report line、confirmed commitment、explicit business expense、confirmed reimbursement與candidate disclosure | `FC-A1`；現有mapping／commitment／reimbursement contracts | 固定事實與heuristic分開；gross／recovery／net可對回交易；未設定essential policy時顯示unknown | 不讓AI每月判「可不可以省」；不把proposal先扣除 |
...
執行優先序原為`FC-A1 → R16 → FC-A2／FC-A3`；FC-A1、R16、FC-A2、FA-0、FC-A3、FC-2與FC-3 v0已完成第一輪code／synthetic closure，下一個runtime package是以真實問題驗收compact Context Pack與AI提示詞。`FC-A4`等待第二個可信position或所選期初已完整後再做。Safe-to-spend、essentiality與dependable-income policy仍留在FC-4，不為了讓近期報表看起來完整而提前猜值。
...
**2026-07-17 FC-A3 execution record：Implemented v0；formal-data acceptance pending。** 新增`spending-structure` behavior contract、query／API、Control consumer、`spending_structure` named dataset、capability advertisement、Operator Skill recipe與2個focused tests。每次查詢重用管理損益、confirmed commitments與reimbursement owners，分開支出科目、explicit business operating expense、confirmed recovery、unallocated與proposal；不把交通／住宿／商家名稱猜成工作費，也不計算essentiality／safe-to-spend。沒有schema migration、report persistence或正式DB write。

**2026-07-17 FC-2 execution record：Implemented v0；formal-data acceptance pending。** 新增`commitment-and-liability-contract`的FC-2投影邊界、`project-obligations` pure projector、typed owners組合query／API、`obligation_timeline` named dataset、Control consumer、Operator Skill API／recipe／prompt references與2個focused test files。輸出card statement／installment、loan schedule與confirmed commitment的7／30／90日events，分開exact／range／unknown、reliability、blockers、source facts與stable event key；card payment只作settlement，loan components必須精確對回付款額。這是query-time read model，不新增canonical table、不持久化forecast，也不宣稱safe-to-spend；browser acceptance與正式資料owner驗收仍待後續。沒有schema migration、正式DB write或AI provider依賴。

**2026-07-17 FC-3 execution record：Implemented v0；formal-data acceptance與owner policy pending。** 新增policy-aware `project-cash-timeline`、trusted opening cash adapter、`cash_forecast` named dataset、唯讀API、`CashTimeline` Control consumer、cash forecast contract、Operator Skill recipe／prompt boundary與2個focused test files。runtime只納入同幣別、active／included且snapshot status為`current`的流動現金；缺snapshot、stale、conflict或needs-review不補零；events只重用FC-2，range／unknown與不確定收入維持排除／揭露。raw path會回傳daily closing與minimum projected cash，但reserve、dependable income、safe-to-spend、alerts與投資／還貸建議仍不可用。沒有schema migration、forecast persistence、正式DB write或AI provider依賴。
...
結論：FA-0、FC-A3、FC-2與FC-3已證明「先由工具準備compact deterministic context，再讓AI解釋」的工作方式可行；下一個切片不是再擴張一套完整版，而是用真實問題驗收Context Pack、補跨context不重複測試，再依owner使用結果決定net-worth bridge或後續policy。
...
**2026-07-17 real-data Context Pack acceptance probe：Read-only completed；owner acceptance pending。** 以正式DB的`2026-07-17`、`personal`、`TWD` scope直接重跑read models與named analysis context：`spending_history`為`complete`；`financial_health_review`、`spending_structure`、`obligation_timeline`與`cash_forecast`均為`partial`，且沒有猜成零。當時blockers包含負債schedule、固定義務到期日、外幣raw forecast與台股aggregate identity；最後一項已由下方2026-07-21紀錄取代，其餘缺口仍有效。

**2026-07-21 consultation-readiness acceptance：Implemented and browser-verified；owner policy仍待後續。** 正式資料已用governed API把owner-confirmed台股position連到canonical `00675L`與dated official quote；重複instrument已停用。`/control`只在request同時帶明確instrument scope與2× factor時顯示情境曝險與10%／20%壓力，並把factor標為「此頁情境」，不寫回canonical facts。真實資料桌機流程已逐頁驗證總覽、損益、資產負債、現金流與投資估值；Balance Sheet可供諮詢，P&L與Cash Flow仍誠實揭露待分類流量與期初boundary缺口。完整`npm run verify:release`為235／235 Node、7／7 Chromium、18／18 Skill eval並通過所有release gate。下一步仍是用真實諮詢問題驗收AI提示詞；不是先擴張完整版或把safe-to-spend寫成硬公式。
