...
Post-audit closure addendum（2026-07-17 foundation line）：repository code與正式DB已到schema v10，完成reimbursement owner、obligation reversal lifecycle、versioned transfer decisions、12個當時已發布的governed analysis datasets、proposal envelope、unified impact review workbench、server-backed management P&L／Balance Sheet／Cash Flow、FC-A2 Monthly Financial Pulse，以及source-conflict review context。正式v6→v9與v9→v10都先建立可驗證backup、在temporary restore演練，再執行formal migration；1,078筆交易與交易雜湊保持不變，`integrity_check=ok`、0 FK violations。代表性card／liability／commitment facts與1筆reimbursement proposal已透過typed APIs進入正式DB。該階段證據包含Node suite 209/209、Skill eval 18/18、Chromium 7/7、production audit／build／runtime／privacy／backup restore全部通過；R16 credit-card lifecycle與FC-A2另以synthetic fixture驗證，本輪未讀寫正式DB。剩餘gate僅為owner scope／proposal confirmation、真實信用卡statement acceptance與GATE-F6 acceptance；2026-07-15與本輪早期的v6／v9數量只保留作historical ledger，最新FA-0／FC-2／FC-3 evidence見下方addendum與`CURRENT-STATUS.md`。

Real-data position addendum（2026-07-16晚間）：在新schema v10備份與副本演練通過後，正式匯入92筆主要信用卡未出帳（72筆連結既有經濟事件、20筆新增）及10筆即時授權，並補主要卡片current-liability與一個低活動現金帳戶的same-date snapshot。正式DB現為1,108 transactions、28 sources、12 balance snapshots；`integrity_check=ok`、0 FK violations。Balance Sheet由缺current card balance的partial提升為`complete`且blockers 0；cash flow仍因歷史cash boundaries與mapping／matching不足維持partial。精確私人金額只保存在ignored intake文件與正式DB。

FA-0 closure addendum（2026-07-17）：`FA-0 Financial Health Review v0`已完成query、API、behavior contract、capability advertisement、Operator Skill handoff、synthetic fixture與3個focused tests。完整`npm run verify:release`在新增測試後以212／212 Node tests、18／18 Skill eval、Chromium、build、runtime、privacy、backup／restore等gate通過；本輪未修改schema、測試以外的正式資料或report persistence。另以read-only connection核對正式DB的FA-0輸出，結果為`partial`，缺口包含loan schedule、可靠收入與必要支出；投資factor exposure需以request明確指定canonical instrument scope與factor，不能把aggregate holding的身分或曝險假設寫成事實。這證明「canonical facts → deterministic metrics → compact AI Context Pack → AI interpretation／options → human decision」切片可行，但不代表已完成safe-to-spend、forecast或完整Control Center。

Consultation-readiness addendum（2026-07-21）：完成真實資料桌機瀏覽器逐頁驗收與資訊呈現修正。當月／未來月份不再預設到晚於今天；資產負債表使用資料庫治理的中文帳戶／機構／投資名稱，並以「淨資產（資產扣除負債）」取代內部或未命名列。Owner-confirmed台股position已連到canonical `00675L`與dated official quote；2× factor只在明確request scope下作頁面情境，不寫回canonical facts。損益與現金流保留4筆待釐清流量及缺期初boundary的partial狀態，不把unknown顯示成0。完整`npm run verify:release`以235／235 Node tests、7／7 Chromium、18／18 Skill eval，以及lint／audit／build／runtime／privacy／backup restore／screenshots全數通過；真實DB另以read-only檢查確認schema v10、`integrity_check=ok`與0 FK violations。
...
**Confirmed：** 整體產品仍未抵達長期終點。Formal management Balance Sheet／Cash Flow已實作；07-16 Balance Sheet已complete，但Cash Flow與部分歷史P&L／reconciliation仍partial。FA-0已提供第一個runtime deterministic health Context Pack，但foundation-to-forecast adapter、safe-to-spend、alerts與scenario lifecycle尚未實作。`projectCashTimeline`仍只對explicit synthetic inputs計算，FA-0也不應被描述為使用者現有財務預測。
...
原始repository audit沒有讀取`data/`、`uploads/`、`outputs/`或statement files中的真實財務內容；所有新tests與browser flows使用synthetic／anonymous isolated DB。2026-07-16晚間的owner-authorized real-data operation則明確讀取兩份主要信用卡CSV並更新ignored private DB／intake文件，沒有把金額或原始檔加入Git。Release privacy gate掃描tracked與untracked、未被Git ignore的`.js`／`.jsx`／`.mjs`／`.json`／`.md`。
...
| Pages | 9：overview、transactions、reports、control、data、trend、corrections、rules、confirmations |
...
| Tests | 69個`.test.js` files，加3個Playwright specs／7個Chromium cases |
...
- 8 readiness goals、15 named analysis datasets、proposal envelope、source watermarks與response limits。
...
- **Remaining gap：** 正式migration已完成，仍需owner acceptance；current card balance與低活動現金帳戶的當前快照已補齊，因此Balance Sheet已complete。Cash Flow與歷史P&L／reconciliation仍因部分boundary、loan／transfer matches及歷史card normalization closure而維持partial／unreconciled。這是資料coverage，不是程式把未知補成0。
...
- **Additional v0 capability：** `FA-0 Financial Health Review`已以query-time方式重用position、liability／card與investment owners，輸出position、liquidity、debt、明確factor exposure、stress與coverage Context Pack；不把AI心算或request assumption寫回canonical facts。
- **Remaining next-stage gaps（superseded by current addendum）：** reliable income／essential spend inputs、net-worth bridge、safe-to-spend、API／UI決策流程與alert lifecycle仍未完成。FC-2 obligation timeline與FC-3 raw forecast已完成第一版，但正式資料與owner policy acceptance仍待驗收；Owner financial policies延後到相應consumer，不列為目前foundation blocker。
...
- **Current strength：** 69個Node test files、3個Playwright specs／7個Chromium cases、18-case skill eval、production build、runtime smoke、privacy scan、anonymous demo與backup rehearsal均納入整合驗證。
...
| Readiness／analysis | Implemented；15 named datasets＋FA-0 read model＋proposal envelope | readiness policy、analysis registry／query／API tests、financial-health route／tests |
...
| Financial Control | Phase 0 reference＋FC-A2 Monthly Pulse＋FA-0 Financial Health v0 | `lib/queries/finance/control/**`、control APIs、contracts、fixtures與focused／browser tests；完整forecast／safe-to-spend／alerts仍未完成 |

## Highest remaining risks and debt

1. **P1 owner gate：** formal migration與postflight已完成；目前沒有待人確認的proposal，但scope attestations與owner最終接受尚未完成，不能宣稱Gate F關閉。
...
3. **P1 forward control：** pure projector、Monthly Pulse與FA-0目前都不具備完整future-obligation timeline／owner policies，不能回答真實risk date或safe-to-spend。
...
1. **由owner關閉MP-07 human gate。** 在`/confirmations`完成仍缺的scope attestation，再用常用問題驗收三張表的partial／known gaps；目前沒有待裁決的typed proposal，AI仍不得代按browser confirmation。
...
4. **Gate F接受後才擴大Financial Control。** 先用已完成的FA-0與Monthly Pulse收集真實使用回饋，再重用現有Balance Sheet facts補commitment／liability timeline；不得另建第二套帳戶、資產、負債或投資資料。
...
以下是原始稽核基線的驗證紀錄；2026-07-21最新重驗與release gate結果以本文件後面的「Current verification addendum」為準。
...
| `node --test` | PASS，212／212，0 fail／skip／todo | legacy、foundation、semantics、reports、review、FA-0與schema v10全通過 |
| Playwright Chromium E2E | PASS，7／7 | Data Center＋三張reports＋Monthly Pulse＋unified workbench的typed owner／stale／partial／retry flows |
| external AI skill eval | PASS，17／17 | operator fixed／adversarial cases通過 |
| Next production build | PASS | 84 API routes與74 static-generation entries可build；FA-0 route已出現在production route map |
...
| Markdown inventory／links／fences | PASS | 78個current Markdown files；missing relative links 0、unbalanced fences 0 |
| `git diff --check` | PASS | 0 whitespace error；package files只有既有line-ending warning，非diff error |
| delivery inventory | PASS with expected scope | source／docs／tests／Skill變更均屬MP-00–06、R16與FA-0；無generated DB／build output／formal data |
...
- 未用完整正式財務資料完成owner acceptance；本輪只以read-only connection核對FA-0，因此仍不知道完整source coverage、資料品質、規模或owner對真實財務結果的接受程度。
...
上述限制不影響「foundation完成」「本輪stabilization完成」「FC-A2與FA-0是bounded runtime read models」「formal forecast／safe-to-spend與完整Control Center未完成」「loopback是安全前提」等直接source結論，但限制對真實資料完整度、易用性、規模與production readiness的判斷。
...
- [x] CodeGraph sync與bounded browser evidence完成；[x] 最新release gate全綠（2026-07-21為Node 235／235、Chromium 7／7、Skill eval 18／18，其他release checks全數通過）。
...
## Current implementation addendum（2026-07-17）

FC-2 Obligation Timeline已在原稽核基線之後完成第一版：新增query-time pure projection、card／loan／commitment composition query、API、14th governed analysis dataset、Control consumer與Operator Skill references。它已取代「trusted obligation timeline尚未完成」的現況描述，但仍只提供7／30／90日已知義務事件，不是現金forecast、safe-to-spend或owner policy。後續真正未完成的forward-control工作是FC-3 raw cash forecast adapter／UI，以及更後面的policy／alert／scenario lifecycle；正式資料接受仍受owner scope、card lifecycle與coverage gaps約束。
FC-3 Raw Cash Forecast已接續完成第一版：新增policy-aware pure projector、trusted opening cash adapter、15th governed analysis dataset、API、Control consumer與Operator Skill forecast boundary。它只使用current同幣別流動現金與FC-2已知義務；缺snapshot不補零，reserve／dependable income／safe-to-spend仍保持unavailable。後續主要工作轉為真實資料owner acceptance、跨context invariant與AI Context Pack提示詞驗收。

## External GPT discussion cross-check addendum（2026-07-21）

本次收到的外部 GPT 討論將產品問題重新整理成「raw evidence → financial events → postings／allocations → statements／AI interpretation」的成熟架構。重新對照 working tree 後，結論是：目前個人財務 foundation 的實作方向符合現階段目標，但不能誤稱為完整事件總帳或企業會計系統。

- **Confirmed：** `sources`／ingestion／data-change evidence、typed financial-event semantics、card／loan／transfer／reimbursement matching、三張server-backed management reports、query-time Control read models、FA-0 compact Context Pack與AI human-confirmation boundary均有程式、contract與tests支援。
- **Confirmed boundary：** 沒有 persisted `financial_events`、double-entry `postings`／journal、通用多對多allocation、customer／project／invoice／AR／AP或personal／business／combined elimination；相關能力在active contracts／capabilities中是non-goal或unsupported，而不是被漏記成Implemented。
- **Partial：** `reporting_entities`、`receivable`／`payable` account kinds與typed allocations提供未來延伸的基礎，但不等於完整企業收入認列、平台gross／fee／net payout或應收應付生命週期。
- **Current decision：** 不在本階段重做foundation或加入完整double-entry。下一個可驗證切片仍是用真實問題驗收FA-0／Monthly Pulse／三張表送給AI的Context Pack；企業財務層需另有owner決策與獨立contract。

逐項表格、證據路徑、CodeGraph核對與本次限制見[`AI-DISCUSSION-ARCHITECTURE-CROSSCHECK.md`](AI-DISCUSSION-ARCHITECTURE-CROSSCHECK.md)。

## Current verification addendum（2026-07-21）

- CodeGraph從`D:\_CabLate_Agents\general\finance-viewer`執行`sync .`後，index為335 files、3,200 nodes、7,522 edges，pending changes為0。
- `npm test`：PASS，235／235。
- `npm run lint`：PASS，0 warnings。
- `npm run build`：PASS，Next.js production build成功。
- `git diff --check`：PASS；本次文件變更沒有whitespace error。
- 最新 `npm run verify:release`：整體 **PASS，exit 0，130.2秒**；Node tests 235／235、Chromium 7／7、Skill eval 18／18，並通過lint、production dependency audit、77-page build、runtime smoke、privacy、anonymous demo、backup／restore rehearsal與screenshot checks。
- 先前失敗的2個dashboard案例已改為驗收目前`FinancialDashboard`的loading／error／partial與URL assumption pass-through；Data Center案例改為驗收人類可讀的「日圓」label。產品語意未為測試退回內部英文代碼。
- Release verifier使用隔離synthetic DB，未開啟或寫入 `data/finance.sqlite`；真實資料另以唯讀API、browser與integrity／FK檢查驗收，兩類證據仍分開記錄。
