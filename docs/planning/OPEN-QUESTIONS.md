...
- **Status：Deferred。** Raw forecast可以只投影trusted opening cash與已知義務，uncertain income維持排除；只有要把未來收入納入base projection或計算safe-to-spend時才需要本決策。

### OQ-7 — Statement與Control優先順序

- **Resolved implementation fact：** Management Balance Sheet與direct-method Cash Flow已由Gate F MP-05完成；Control不得重寫statement或建立第二套position truth。
- **Recommended execution order：** MP-07 owner acceptance → consume existing position facts → obligation timeline → read-only Control MVP → raw deterministic forecast → policy／safe-to-spend／alerts。
- **Status：Control Center整體在foundation closure後已由owner確認；內部分序仍是canonical Master Plan的proposed execution order。** 啟動第一個Control package時重新驗證依賴即可，不需要為已完成statement再做一次owner選擇。
...
- 非技術使用者首次建檔成功率。
- 已建立一份2026-07-16 verified backup並完成temporary restore／migration rehearsal；長期backup是否定期執行仍無法確認。
